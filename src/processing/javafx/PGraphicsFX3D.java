...
public class PGraphicsFX3D extends PGraphicsJavaFX {
    ...
    Path2D workPath = new Path2D.Float();
    Path2D auxPath = new Path2D.Float();
    boolean openContour;
...
//  }
    ...
    public void vertex(float x, float y) {
        ...
        switch (shape) {
            ...
            case POLYGON:
                if (workPath.getCurrentPoint() == null || breakShape) {
                    workPath.moveTo(x, y);
                    breakShape = false;
                } else {
                    workPath.lineTo(x, y);
    ...
    public void beginContour() {
        ...
        if (contourPath.getCurrentPoint() != null) {  // first contour does not break
            breakShape = true;
        }
    ...
    public void endContour() {
        ...
        if (workPath.getCurrentPoint() != null) workPath.closePath();
    ...
    public void endShape(int mode) {
        ...
        if (workPath.getCurrentPoint() != null) {
            if (shape == POLYGON) {
                if (mode == CLOSE) {
                    workPath.closePath();
                }
                if (auxPath.getCurrentPoint() != null) {
