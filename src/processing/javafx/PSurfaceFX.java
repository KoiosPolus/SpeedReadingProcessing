...
public class PSurfaceFX implements PSurface {
    ...
    public PSurfaceFX(PGraphicsJavaFX graphics) { //generalized to allow initialization by 2D or 3D
        fx = graphics;
...
//        static double getRenderScale(Screen screen) throws NoSuchMethodException, SecurityException, IllegalAccessException, IllegalArgumentException, InvocationTargetException, InvocationTargetException {
//            Method m;
//            try {
...
//        }
//        static double getUIScale(Screen screen) throws NoSuchMethodException, SecurityException, IllegalAccessException, IllegalArgumentException, InvocationTargetException, InvocationTargetException {
//            Method m;
//            try {
//                m = Screen.class.getDeclaredMethod("getUIScale"); // until 8u60 b15
//                m.setAccessible(true);
//                return ((Float) m.invoke(screen)).doubleValue();
//            } catch (NoSuchMethodException e) {
...
//        }
        ...
        @Override
        public void start(final Stage stage) throws NoSuchMethodException, IllegalAccessException, InvocationTargetException {
//      if (title != null) {
...
//      }
            ...
            float renderScale = (float) Screen.getPrimary().getDpi();
            if (PApplet.platform == PConstants.MACOSX) {
                for (Screen s : Screen.getScreens()) {
                    renderScale = (float) Math.max(renderScale, s.getDpi());
                }
            }
            float uiScale = (float) renderScale;
    ...
    public void initFrame(PApplet sketch) {/*, int backgroundColor,
        ...
        new Thread(() -> Application.launch(PApplicationFX.class)).start();
